...
public class PlayerListener extends PListener {
    ...
    public void onCatalPlace(PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock().getType() == Material.OBSIDIAN && (slot = this.getAltar(e.getClickedBlock())) != -1) {
            ...
            SkySimEngine.getPlugin().getSummoningSequence().interactCatalyst(e.getPlayer(), slot, true);
    ...
    public void onMoveWorld(PlayerChangedWorldEvent e) {
        ...
        if (SkySimEngine.getPlugin().getSummoningSequence() != null && !SkySimEngine.getPlugin().getSummoningSequence().isBossSpawned() && !SkySimEngine.getPlugin().getSummoningSequence().isBossSpawning()) {
            SkySimEngine.getPlugin().getSummoningSequence().pickupAllCatalysts(player);
    ...
    public void playAltarExplosionEffects() {
        Block[] altarList = Altar.altarList;
        for (Block block : altarList) {
    ...
    boolean isAltar(Block b) {
        ...
        for (Block block : altarList) {
            ...
            Location bl2 = block.getLocation();
    ...
    public void updateCatalystsBlock(World w) {
        ...
        Block[] a = Altar.altarList;
        UUID[] cache = SkySimEngine.getPlugin().getSummoningSequence().__qch__;
        for (int i = 0; i < a.length; ++i) {
    ...
    void spawnCatalCore(Location loc, final int catalystID) {
        ...
        loc.add(0.5, 0.0, 0.5);
        final ArmorStand core = loc.getWorld().spawn(loc.clone().add(0.0, -1.2, 0.0), ArmorStand.class);
    ...
    boolean s(int z) {
        if (z == 0 || z == 1) {
            return SkySimEngine.getPlugin().getSummoningSequence().isAcD();
        }
        if (z == 2 || z == 3) {
            return SkySimEngine.getPlugin().getSummoningSequence().isAcE();
        }
        if (z == 4 || z == 5) {
            return SkySimEngine.getPlugin().summoningSequence.isAcG();
        }
        if (z == 6 || z == 7) {
            return SkySimEngine.getPlugin().summoningSequence.isAcR();
        }
        return false;
    }
