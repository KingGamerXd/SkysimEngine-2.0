...
extends PListener {
    ...
    public final static ArrayList<Material> BLACK_LIST = new ArrayList<>();
    ...
    public void onPortalEnter(EntityPortalEnterEvent e) {
        ...
        if (portalType == Material.PORTAL) {
            World hub = Bukkit.getWorld(!this.plugin.config.getString("hub_world").isEmpty() ? this.plugin.config.getString("hub_world") : "hub");
            if (hub == null) {
    ...
    private static void applySlowDiggingEffectToPlayers() {
        new BukkitRunnable(){
            ...
            public void run() {
                Bukkit.getOnlinePlayers().forEach(p -> p.addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 1000000, 255)));
            }
        }.runTaskTimer(SkySimEngine.getInstance(), 0L, 1L);
    }

    public static void initialize(){
        applySlowDiggingEffectToPlayers();
        ...
        registerBlockDigPacketListener();
    }

    private static void registerBlockDigPacketListener() {
        SkySimEngine.getProtocolManager().addPacketListener(new PacketAdapter(SkySimEngine.getPlugin(), ListenerPriority.HIGHEST, PacketType.Play.Client.BLOCK_DIG){
    ...
    public double findMiningSpeedFor(Player p, Block b) {
        ...
        if (BLACK_LIST.contains(b.getType())) {
